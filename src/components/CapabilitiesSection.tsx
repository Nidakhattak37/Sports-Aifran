...
export const CapabilitiesSection: React.FC = () => {
  ...
  return (
    <section id="capabilities" className="py-20 bg-[#070A0F] text-white border-b border-slate-800/80">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {CAPABILITIES_DATA.map((item) => (
            <div
              ...
              className="bg-[#0D1322]/90 border border-slate-800/90 hover:border-amber-500/50 hover:bg-[#11192e] rounded-2xl p-5 sm:p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_30px_rgba(234,88,12,0.15)] group cursor-pointer flex flex-col justify-between space-y-4"
            >
              <div className="space-y-3">
                ...
                <h3 className="text-xs sm:text-sm font-extrabold font-['Sora'] text-white uppercase tracking-tight group-hover:text-amber-400 transition-colors leading-snug">
                  {item.title}
                </h3>
          ...
          <div className="bg-[#0B0B0B] border border-slate-700/80 rounded-2xl max-w-xl w-full p-6 sm:p-8 space-y-6 shadow-2xl relative text-white">
            ...
            <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 space-y-3 font-mono text-xs">
              <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                ...
                <span className="text-amber-400 font-bold">{selectedCapability.equipmentType || 'Juki Industrial Equipment'}</span>
              </div>

              <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                <span className="text-slate-400 uppercase">DAILY OUTPUT CAPACITY:</span>
                <span className="text-white font-bold">{selectedCapability.dailyOutput || 'High capacity'}</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-slate-400 uppercase">PRECISION TOLERANCE:</span>
                <span className="text-emerald-400 font-bold">{selectedCapability.precisionTolerance || 'MIL-SPEC compliant'}</span>
              </div>
            </div>
